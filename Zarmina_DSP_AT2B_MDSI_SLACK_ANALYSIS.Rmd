...
# Conclusion
The  MDSI slcakdataset has huge information (both structured and unstructured) stored in it, that can be analysed in multiple different ways to get different insights from it, if that is in the form of analysing any specific channel, the sentiments of users for in different channels and several other important analysis (qualitative as well as quantitative).
The analysis and key insights in this task are:

* **From slack data:** There are four data tables present in the MDSI slack dataset, each having unique variables. from all those tables a user_tect_channel1 dataset is created that contains information about the users, their messages, channel ownership and its respective channels, for anaylsis in R and python.
* **Using Python:** The characteristic features, feature type, data dimensioanlity,statistical characteristics of features and distribution of 0observations of categorical variables.
* **USing R:** R is used for visualizations and getting insights from the user_text_channel1 dataset created in R. Using R, we analysed that what are the most frequent words used in **MDSI_DAM_AUT_19** channel, apart from that which user has the highest message frequency (barplot), and also if the user is owner of any channel or not. From this analysis we came to know that Perry Stephenson and Alex Scriven have got the highest message frequency (channel owners as well).
